...
//    };
    ...
    struct BubbleMap
    {
        ...
        void addCoord( int match, int bubble, int len );
        ConMap* map_;
        Match* match_;
...
//        vector<IntraBubble*> bubbles_;
    ...
    {
//        BubbleAltPath(){};
        BubbleAltPath( BubbleMapDetails& map ): seq_( map.seq_ ), map_( 1, map ), type_( map.type_ ){};
        bool add( BubbleMapDetails& map );
        void assign( int* coords );
        bool bridge( Bubble* b, BubbleAltPath& rhs, vector<BubbleAltPath>& paths );
        static vector<BubbleAltPath> build( Bubble* b );
        bool compatible( Bubble* b, SnpAlignResult::BubbleAlignCoords& bac );
        ...
        bool presplit( Bubble* base, SnpAlignResult::BubbleAlignCoords& bac );
        ...
        int cut_[2], coord_[2], score_, type_;
    };
    ...
    void addCloned( Bubble* b );
    void addCloned( BubbleMap bm, int start, int len, int offset );
...
//    vector<Bubble::BubbleAltPath> getAlignableSeqs();
    string getConsensus();
    vector<ConMap*> getMapped();
    void getMapRange( ConMap* cm, int& start, int& end );
    bool getOffset( ConMap* l, ConMap* r, int& off );
//    void getSeq( BubbleMap& map );
//    vector<pair<string,vector<Bubble*>>> getAlignableSeqs();
    bool isAnchored();
    bool isClash( Bubble* b );
    bool isClash( BubbleCoord& bc );
    bool isFoldTarget( ConMap* cm, int (&limits)[2], bool finalised, bool drxn );
    bool remove( ConMap* cm, bool doErase=false );
    void remove( unordered_set<ConMap*>& removes );
    bool retract( vector<ConMap*>& orphans );
    ...
    vector<Bubble*> split( vector<Bubble*> stacks[2], int coord[2], int anchor );
    Bubble* stealBubble( unordered_set<ConMap*>& ignores, int startOffset );
    void test();
