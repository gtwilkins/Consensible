...
uint64_t getKmer( string& seq, int i, int len )
{
    uint64_t k = charToInt[ seq[i] ];
    for ( int j = 1; j < len; j++ ) k = ( k << 2 ) | charToInt[ seq[i+j] ];
    return k;
}

vector<CharId> getKmers( string& seq, int len )
{
    assert( len <= 32 );
    int coord = -len;
    CharId k = 0, mask = 0;
    vector<CharId> kmers;
    for ( int i = 0; i < len; i++ ) mask = ( mask << 2 ) + 3;
    for ( int i = 0; i < seq.size(); i++ )
    {
        k = ( ( k << 2 ) + charToInt[ seq[i] ] ) & mask;
        if ( ++coord >= 0 ) kmers.push_back( k );
    }
    return kmers;
}
